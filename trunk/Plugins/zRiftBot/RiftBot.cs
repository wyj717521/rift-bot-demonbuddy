...
namespace RiftBot
{
    ...
    public static class Logger
    {
        ...
        public static void Log(string message, params object[] args)
        {
            ...
            Logging.InfoFormat("[RiftBot] " + string.Format(message, args), type.Name);
        }
    ...
    public class TrinityExploreRift : Trinity.XmlTags.TrinityExploreDungeon
    {
        ...
        private Composite isRiftExploreCompleted()
        {
            ...
            new PrioritySelector(
				...
				new Decorator(ret => EndType == TrinityExploreEndType.RiftComplete && ExitNameHash == 1 && isHashVisible(),
					new Sequence(
						new Action(ret => Logger.Log("Rifting, exit found. Tag Finished.")),
						new Action(ret => _isDone = true)
					)
				),
				new Decorator(ret => EndType == TrinityExploreEndType.RiftComplete && ExitNameHash > 1 && isHashMet(),
					new Sequence(
						new Action(ret => Logger.Log("Rifting, exit found {0}. Tag Finished.", ExitNameHash)),
						new Action(ret => _isDone = true)
					)
				)
			);
		...
		private bool isHashMet()
        {
            return ZetaDia.Minimap.Markers.CurrentWorldMarkers.Any(m => ExitNameHash == m.NameHash && m.Position.Distance2D(Trinity.Trinity.Player.Position) <= MarkerDistance + 20f);
        }
